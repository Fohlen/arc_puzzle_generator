...
from arc_puzzle_generator.rule import RuleNode, TerminateAtPointRule, CollisionConditionRule
...
def puzzle_eightysix(input_grid: np.ndarray) -> Playground:
    ...
    for i in range(1, num_objects + 1):
        ...
        agents.append(Agent(
            ...
            charge=-1,
            node=RuleNode(
                TerminateAtPointRule(PointSet([start_point]), direction_rule=identity_direction),
                alternative_node=RuleNode(
                    TerminateAtPointRule(PointSet([start_point]), direction_rule=clockwise_direction_90),
                    alternative_node=RuleNode(
